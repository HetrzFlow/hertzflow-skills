...
Usage:
    python3 cross_llm_diff.py <model_a_filled.json> <model_b_filled.json>
"""
...
def main() -> int:
    if len(sys.argv) != 3:
        print("Usage: cross_llm_diff.py <model_a_filled.json> <model_b_filled.json>", file=sys.stderr)
        return 1

    model_a = json.loads(Path(sys.argv[1]).read_text(encoding="utf-8"))
    model_b = json.loads(Path(sys.argv[2]).read_text(encoding="utf-8"))

    model_a_leaves = dict(_flatten(model_a))
    model_b_leaves = dict(_flatten(model_b))

    # Categorize differences
    only_model_a = set(model_a_leaves) - set(model_b_leaves)
    only_model_b = set(model_b_leaves) - set(model_a_leaves)
    common = set(model_a_leaves) & set(model_b_leaves)
    identical = [p for p in common if model_a_leaves[p] == model_b_leaves[p]]
    differ = [p for p in common if model_a_leaves[p] != model_b_leaves[p]]
    ...
    model_a_placeholder_paths = [p for p, v in model_a_leaves.items() if v == PLACEHOLDER]
    ...
    print("=" * 72)
    print(f"Total leaves: model_a={len(model_a_leaves)} model_b={len(model_b_leaves)}")
    ...
    print(f"  only_model_a:         {len(only_model_a)}")
    print(f"  only_model_b:          {len(only_model_b)}")
    print()
    print(f"Placeholder survival:")
    print(f"  model_a has:          {len(model_a_placeholder_paths)}  ← LLM-fill incomplete if > 0")
    ...
    if only_model_a or only_model_b:
        print("PATH PRESENCE MISMATCH (one LLM added/dropped a field):")
        for p in sorted(only_model_a)[:10]:
            print(f"  only model_a: {p}")
        for p in sorted(only_model_b)[:10]:
            print(f"  only model_b:  {p}")
        print()

    # Verdict alignment check
    cv_enum = model_a.get("verdict", {}).get("enum")
    cx_enum = model_b.get("verdict", {}).get("enum")
    print(f"verdict.enum: model_a={cv_enum!r} model_b={cx_enum!r} → "
          f"{'MATCH' if cv_enum == cx_enum else 'DIVERGE'}")

    cv_action = model_a.get("decision_action_block", {}).get("immediate_action", {}).get("action_enum")
    cx_action = model_b.get("decision_action_block", {}).get("immediate_action", {}).get("action_enum")
    print(f"action_enum:  model_a={cv_action!r} model_b={cx_action!r} → "
    ...
    for p in ["verdict.one_liner", "anomaly.verdict_impact", "liq.interpretation"]:
        cv = model_a_leaves.get(p, "<absent>")
        xv = model_b_leaves.get(p, "<absent>")
        print(f"\n  {p}:")
        print(f"    model_a: {str(cv)[:200]}{'...' if len(str(cv)) > 200 else ''}")
    ...
    convergence_ok = (
        ...
        and len(model_a_placeholder_paths) == 0
